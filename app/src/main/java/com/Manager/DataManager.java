...
public class DataManager{
	...
	private static class DataManagerHolder {
		private static final DataManager INSTANCE = new DataManager(MyApplication.getInstance());
	}
	...
	public boolean addDev(DevModel model){
		...
		try {
			...
			contentValues.put("ip", model.ip);
			contentValues.put("dataport", model.dataport);
			contentValues.put("httpport", model.httpport);
			contentValues.put("usr", model.usr);
			contentValues.put("pwd", model.pwd);
			contentValues.put("resolute", model.resolute);
	...
	public boolean updateDev(DevModel model){
		...
		try {
			...
			cv.put("httpport", model.httpport);
			cv.put("usr", model.usr);
			cv.put("pwd", model.pwd);
			cv.put("resolute", model.resolute);

			Log.e(TAG, "update in manager sn:" + model.sn + ",resolute" + model.resolute);
	...
	private DevModel parseNode(Cursor cursor){
		...
		model.ip = (cursor.getString(cursor.getColumnIndex("ip")));
		model.dataport = (cursor.getString(cursor.getColumnIndex("dataport")));
		model.httpport = (cursor.getString(cursor.getColumnIndex("httpport")));
		model.usr = (cursor.getString(cursor.getColumnIndex("usr")));
		model.pwd = (cursor.getString(cursor.getColumnIndex("pwd")));
		model.resolute = (cursor.getInt(cursor.getColumnIndex("resolute")));
		return model;
	}
